...
def test_mysql_innodb_backup(backup_location, ops_test, tmp_dir: Path):
    ...
    output = subprocess.check_output(
        f"juju-backup-all -l DEBUG -o {tmp_dir} -e etcd -e postgresql -x -j --backup-location-on-mysql {backup_location}",
        shell=True,
    )
...
def test_postgresql_backup(backup_location, ops_test, tmp_dir: Path):
    ...
    output = subprocess.check_output(
        f"juju-backup-all -l DEBUG -o {tmp_dir} -e etcd -e mysql-innodb-cluster -x -j --backup-location-on-postgresql {backup_location}",
        shell=True,
    )
    output_dict = json.loads(output)
...
def test_etcd_backup(backup_location, ops_test, tmp_dir: Path):
    ...
    output = subprocess.check_output(
        f"juju-backup-all -l DEBUG -o {tmp_dir} -e mysql-innodb-cluster -e postgresql -x -j --backup-location-on-etcd {backup_location}",
        shell=True,
    )
    output_dict = json.loads(output)
...
def test_juju_client_config_backup(tmp_dir: Path):
    output = subprocess.check_output(
        f"juju-backup-all -l DEBUG -o {tmp_dir} -e etcd -e mysql-innodb-cluster -e postgresql -x",
        shell=True,
    )
    output_dict = json.loads(output)
