...
def test_mysql_innodb_backup(backup_location, ops_test, tmp_dir: Path):
    ...
    output = subprocess.check_output(
        f"juju-backup-all -o {tmp_dir} -e etcd -e postgresql -x -j --backup-location-on-mysql {backup_location}",
        shell=True,
    )
    output_dict = json.loads(output)
    expected_output_dir = tmp_dir / controller_name / model_name / mysql_innodb_app_name
    app_backup_entry = output_dict.get("app_backups")[0]
    assert any(str(tmp_dir) in x.get("download_path") for x in output_dict.get("app_backups"))
...
def test_postgresql_backup(backup_location, ops_test, tmp_dir: Path):
    ...
    output = subprocess.check_output(
        f"juju-backup-all -o {tmp_dir} -e etcd -e mysql-innodb-cluster -x -j --backup-location-on-postgresql {backup_location}",
        shell=True,
    )
    output_dict = json.loads(output)
    expected_output_dir = tmp_dir / controller_name / model_name / postgresql_app_name
    app_backup_entry = output_dict.get("app_backups")[0]
    assert any(str(tmp_dir) in x.get("download_path") for x in output_dict.get("app_backups"))
    assert app_backup_entry.get("controller") == controller_name
    assert any(x.get("model") == model_name for x in output_dict.get("app_backups"))
...
def test_etcd_backup(backup_location, ops_test, tmp_dir: Path):
    etcd_app_name = "etcd"
    model_name = ops_test.model.name
    controller_name = ops_test.controller_name
    etcd_app = ops_test.model.applications.get(etcd_app_name)
    output = subprocess.check_output(
        f"juju-backup-all -o {tmp_dir} -e mysql-innodb-cluster -e postgresql -x -j --backup-location-on-etcd {backup_location}",
        shell=True,
    )
    output_dict = json.loads(output)
    expected_output_dir = tmp_dir / controller_name / model_name / etcd_app_name
    app_backup_entry = output_dict.get("app_backups")[0]
    assert any(str(tmp_dir) in x.get("download_path") for x in output_dict.get("app_backups"))
    assert app_backup_entry.get("controller") == controller_name
    assert any(x.get("model") == model_name for x in output_dict.get("app_backups"))
...
def test_juju_controller_backup(ops_test, tmp_dir: Path):
    controller_name = ops_test.controller_name
    output = subprocess.check_output(
        f"juju-backup-all -o {tmp_dir} -e etcd -e mysql-innodb-cluster -e postgresql -j",
        shell=True,
    )
    output_dict = json.loads(output)
    expected_output_dir = tmp_dir / controller_name
    controller_backup_entry = output_dict.get("controller_backups")[0]
    assert str(tmp_dir) in controller_backup_entry.get("download_path")
...
def test_juju_client_config_backup(tmp_dir: Path):
    output = subprocess.check_output(
        f"juju-backup-all -o {tmp_dir} -e etcd -e mysql-innodb-cluster -e postgresql -x",
        shell=True,
    )
    output_dict = json.loads(output)
    expected_output_dir = tmp_dir / "local_configs"
    config_backup_entry = output_dict.get("config_backups")[0]
    assert str(tmp_dir) in config_backup_entry.get("download_path")
